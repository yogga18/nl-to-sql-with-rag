...
#     """
#     Membuat RAG chain lengkap untuk proses Text-to-SQL.
#     """
#     retriever = get_retriever()
    
#     # Kita ingin output yang akurat dan tidak kreatif, jadi temperature=0
#     llm = ChatGoogleGenerativeAI(model="gemini-2.5-flash", temperature=0) 

#     # Prompt Template ini adalah bagian paling krusial
#     template = """
#     Anda adalah seorang ahli pembuat query SQL yang sangat akurat untuk database MySQL.
#     Tugas Anda adalah mengubah pertanyaan pengguna menjadi satu query SQL yang valid untuk tabel bernama `drauk_unit`.

#     PERHATIKAN ATURAN BERIKUT:
#     1. WAJIB menggunakan HANYA kolom dan tipe data yang disebutkan dalam "Konteks Skema" di bawah ini.
#     2. PASTIKAN semua kondisi filter dari "Pertanyaan Pengguna" (seperti tahun, nama unit, sumber dana, dll.) dimasukkan ke dalam klausa WHERE.
#     3. JANGAN mengasumsikan nama tabel atau kolom lain. Gunakan HANYA `drauk_unit`.
#     4. Kembalikan HANYA string query SQL mentah, tanpa penjelasan atau format tambahan seperti ```sql.
...
#     # Merangkai chain menggunakan LangChain Expression Language (LCEL)
#     sql_chain = (
#         {
#             "context": retriever, 
...
# # Validasi pertanyaan dengan klasifikasi sederhana
...
#     1. "data_perusahaan": Jika pertanyaan berkaitan dengan anggaran, realisasi, sisa dana, kegiatan, unit kerja, sasaran strategis, program, atau data internal lainnya yang mungkin ada di database.
#     2. "pengetahuan_umum": Jika pertanyaan adalah tentang topik lain di luar data perusahaan (contoh: 'siapa presiden amerika serikat sekarang?', 'apa ibukota prancis?', dll.).
...
#     # Rangkai chain klasifikasi yang sederhana
...
#     """
#     Membuat chain untuk menganalisis hasil data SQL dan mengubahnya
#     menjadi jawaban dalam bahasa natural.
#     """
#     # Kita bisa sedikit menaikkan temperature untuk jawaban yang lebih luwes
#     llm = ChatGoogleGenerativeAI(model="gemini-2.5-flash", temperature=0.1)

#     template = """
#     Anda adalah seorang analis data AI yang cerdas.
#     Berdasarkan pertanyaan asli dari pengguna dan data hasil query SQL berikut, berikan jawaban dalam satu atau dua kalimat yang informatif dan mudah dimengerti dalam Bahasa Indonesia.
#     Jangan mengulang data dalam format tabel, tetapi rangkumlah temuannya.
#     Jika data kosong, katakan bahwa tidak ada data yang ditemukan untuk pertanyaan tersebut.

#     Contoh:
#     Pertanyaan Asli Pengguna: Berapa total anggaran untuk Direktorat Keuangan?
#     Data Hasil Query:
#          Total Anggaran
#     0    5.000.000.000
#     Jawaban Analisis: Total anggaran untuk Direktorat Keuangan adalah sebesar 5 Miliar Rupiah.
...
#     """
#     Membuat RAG chain untuk Text-to-SQL yang sekarang mendukung riwayat percakapan.
#     """
#     retriever = get_retriever()
#     llm = ChatGoogleGenerativeAI(model="gemini-2.5-flash", temperature=0)

#     template = """
#     Anda adalah seorang ahli pembuat query SQL yang sangat akurat untuk tabel bernama `drauk_unit`.
#     PERHATIKAN RIWAYAT PERCAKAPAN SEBELUMNYA untuk memahami konteks pertanyaan lanjutan.
#     ATURAN PENTING:
#     1. Gunakan HANYA kolom dari "Konteks Skema".
#     2. Untuk permintaan "terbesar" atau "tertinggi", GUNAKAN `ORDER BY ... DESC LIMIT ...`.
#     3. Untuk permintaan "terkecil" atau "terendah", GUNAKAN `ORDER BY ... ASC LIMIT ...`.
#     4. Kembalikan HANYA string query SQL mentah.
...
#     # Mendefinisikan alur data yang lebih eksplisit untuk setiap komponen
#     chain = (
...
#     """
#     Membuat chain analisis yang sekarang juga menggunakan riwayat percakapan untuk konteks.
#     """
#     llm = ChatGoogleGenerativeAI(model="gemini-2.5-flash", temperature=0.2)

#     # --- PROMPT DIPERBARUI ---
#     template = """
#     Anda adalah seorang analis data AI. Berdasarkan pertanyaan asli pengguna, riwayat percakapan, dan data hasil query, berikan jawaban dalam satu atau dua kalimat yang informatif dalam Bahasa Indonesia.
