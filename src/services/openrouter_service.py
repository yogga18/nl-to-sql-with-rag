...
# ======================================================================
# ========== KOMPONEN STATIS (DIBUAT SEKALI SAAT STARTUP) ==========
# ======================================================================
# Komponen di bawah ini akan dibuat HANYA SEKALI saat aplikasi FastAPI Anda dimulai.
# Ini adalah kunci utama untuk peningkatan performa secara drastis.

print("🚀 Menginisialisasi komponen statis (Retriever, Prompts)...")

# 1. Inisialisasi Retriever (Operasi paling 'mahal' dan penting)
RETRIEVER = get_retriever()

# 2. Inisialisasi semua Prompt Template
...
# 3. Inisialisasi daftar model (lebih efisien sebagai konstanta)
AVAILABLE_MODELS_DATA = {
    "popular_models": [
        {"id": "anthropic/claude-3-opus", "name": "Claude 3 Opus", "provider": "Anthropic", "description": "Most capable model, best for complex tasks"},
        {"id": "anthropic/claude-3-sonnet", "name": "Claude 3 Sonnet", "provider": "Anthropic", "description": "Balanced performance and speed"},
        {"id": "anthropic/claude-3-haiku", "name": "Claude 3 Haiku", "provider": "Anthropic", "description": "Fastest and most compact"},
        {"id": "openai/gpt-4o", "name": "GPT-4o", "provider": "OpenAI", "description": "Latest flagship model from OpenAI"},
        {"id": "google/gemini-flash-1.5", "name": "Gemini 1.5 Flash", "provider": "Google", "description": "Fast and cost-effective multimodal model"},
        {"id": "meta-llama/llama-3-70b-instruct", "name": "Llama 3 70B Instruct", "provider": "Meta", "description": "Open source, powerful reasoning"}
    ],
}

print("✅ Inisialisasi komponen statis SELESAI.")

# ======================================================================
# ========== HELPER & WORKFLOW (DIPANGGIL PER-REQUEST) ==========
# ======================================================================
def create_openrouter_llm(model_name: str, temperature: float = 0.0):
    """Helper untuk membuat ChatOpenAI instance dengan OpenRouter config"""
    config = get_openrouter_config()
...
def openrouter_nl_to_sql_workflow(question: str, model_name: str) -> Dict[str, Any]:
    """Workflow lengkap yang dioptimalkan untuk performa."""
    try:
        # Step 1: ROUTER (Optimasi: Gunakan model super cepat untuk tugas sederhana ini)
        llm_router = create_openrouter_llm("anthropic/claude-3-haiku", temperature=0.0)
        router_chain = ROUTER_PROMPT | llm_router | StrOutputParser()
        klasifikasi = router_chain.invoke({"question": question})

        if "pengetahuan_umum" in klasifikasi.lower():
            return {"type": "REJECTED", "answer": "Maaf, saya hanya dapat menjawab pertanyaan terkait data perusahaan...", "model_used": model_name, "step": "router"}

        # Step 2: SQL GENERATION
        llm_sql = create_openrouter_llm(model_name, temperature=0.0)
        sql_chain = (
        ...
        if not sql_query or "error" in sql_query.lower() or len(sql_query) < 10:
            return {"type": "SQL_GENERATION_FAILED", "answer": "Tidak dapat membuat query SQL dari pertanyaan Anda...", "model_used": model_name, "step": "sql_generation"}

        # Step 3: VALIDATION
        if not is_safe_select_query(sql_query):
            return {"type": "UNSAFE_SQL_QUERY", "answer": "Query yang dihasilkan tidak aman...", "generated_sql": sql_query, "model_used": model_name, "step": "validation"}

        # Step 4: EXECUTION
        sql_result_df = execute_sql_query(sql_query)
        if isinstance(sql_result_df, str):
            return {"type": "SQL_EXECUTION_ERROR", "answer": f"Terjadi error saat eksekusi query: {sql_result_df}", "generated_sql": sql_query, "model_used": model_name, "step": "execution"}

        sql_result_for_llm = "Query berhasil dieksekusi, namun tidak ada data yang ditemukan."
        if not sql_result_df.empty:
            sql_result_for_llm = sql_result_df.to_string(index=False)

        # Step 5: ANALYSIS
        llm_analysis = create_openrouter_llm(model_name, temperature=0.1)
        analysis_chain = ANALYSIS_PROMPT | llm_analysis | StrOutputParser()
        final_answer = analysis_chain.invoke({"question": question, "sql_result": sql_result_for_llm})
        ...
        return {
            ...
            "raw_data": sql_result_df.to_dict(orient="records"),
            "data_count": len(sql_result_df),
            "model_used": model_name, "step": "completed"
        }

    except Exception as e:
        return {"type": "ERROR", "answer": f"Terjadi error dalam proses: {str(e)}", "model_used": model_name, "error": str(e), "step": "exception"}

def chat_with_openrouter(prompt: str, model_name: str) -> Dict[str, Any]:
    """Chat bebas dengan OpenRouter yang dioptimalkan."""
    try:
        llm = create_openrouter_llm(model_name, temperature=0.7)
        chain = SIMPLE_CHAT_PROMPT | llm | StrOutputParser()
        response = chain.invoke({"prompt": prompt})
        
        return {"type": "SUCCESS", "answer": response, "model_used": model_name, "status": "completed"}
    
    except Exception as e:
        return {"type": "ERROR", "answer": f"Terjadi error: {str(e)}", "model_used": model_name, "status": "failed", "error": str(e)}

def get_available_models() -> Dict[str, list]:
    """Mengembalikan daftar model populer yang tersedia."""
